...
export async function GET() {
  if (!process.env.DATABASE_URL) {
    return NextResponse.json({ kpis: {}, recommendations: [] });
  } try {
    ...
    // Nếu có lỗi sập mạng xảy ra, trả về mảng rỗng thay vì ném lỗi (giúp vượt qua khâu build Vercel)
    return NextResponse.json({ kpis: {}, recommendations: [], error: "Database lag during build skipped" });
  }
}