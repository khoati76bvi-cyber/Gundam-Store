...
function BarList({ title, rows }: { title: string; rows: any[] }) {
  ...
  return <div className="rounded-3xl border border-white/10 bg-white/[.04] p-5">
    ...
    <div className="space-y-3">{rows.length ? rows.map((r, i: number) => <div key={i}>
...
export default async function AnalyticsPage() {
  ...
  // Đã sửa lỗi ép kiểu any cho s ở đây
  ...
  const purchasedNames = countBy(orders.flatMap((o: any) => o.items.map((i: any) => ({ name: i.name }))), 'name');

  // Đã sửa lỗi ép kiểu any cho x ở hàm định vị .find()
  const lowConversion = topProducts.filter(p => !purchasedNames.find((x: any) => x.name === p.name)).slice(0, 5);
  ...
  return <div>
    ...
    <div className="mt-8 rounded-3xl border border-cyan-300/20 bg-cyan-400/10 p-6">
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">{insights.map((x, i: number) => <div key={i} className="rounded-2xl bg-black/25 border border-white/10 p-4 text-white/80">{x}</div>)}</div>
    </div>
  </div>
}